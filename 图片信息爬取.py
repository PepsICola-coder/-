...
def get_img(save_path , id , bid ,headers, zz):
    ...
    # 获取HTML内容字符串
    html_content = response.json()['data']['goods']["content_web"]

    # 创建BeautifulSoup对象解析HTML
    soup = BeautifulSoup(html_content, 'html.parser')

    # 查找所有img标签并提取src属性中的HTTPS地址
    img_urls = [img['src'] for img in soup.find_all('img') if img.get('src', '').startswith('https://')]
...
for i in range(len(urls)):
    try:
        save_path = os.path.join(save_root, path_name[i])
